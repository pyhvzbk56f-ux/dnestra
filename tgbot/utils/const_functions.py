...
from tgbot.data.config import BOT_TIMEZONE, get_admins, get_operators
...
def is_valid_url(url: str) -> bool:
    """Проверяет, является ли строка допустимым URL. | Args: url (str): URL для проверки | Returns:  bool: True если URL валидный, False если нет"""
    if not url:
        return False

    valid_starts = ("http://", "https://", "tg://")
    return any(url.startswith(prefix) for prefix in valid_starts)
...
async def send_admins(
    bot: Bot, text: str, keyboard: InlineKeyboardMarkup = None, not_me: int = 0, include_operators: bool = False,
):
    """Отправляет сообщение администраторам и, опционально, операторам.
    Args:
        bot: Экземпляр бота.
        text: Текст сообщения.
        keyboard: Инлайн-клавиатура.
        not_me: ID пользователя, которому не нужно отправлять сообщение.
        include_operators: Флаг для включения операторов в рассылку.
    """

    recipients = set(get_admins())
    if include_operators:
        recipients.update(get_operators())
    ...
    for user_id in recipients:
        try:
            ...
            bot_logger.error(f"""Error sending {text} to admin {user_id}: {e}""")
...
def get_unix(nano: bool = False, full: bool = True) -> int:
    ...
    if nano:
        ...
        # Время в секундах с 00:00 текущего дня
        else:
            return int(
                datetime.now()
...
# Конвертация дней
def convert_day(day: int, i18n: Translator, locale: str) -> str:
    day = int(day)
    days = [
        i18n.get_text("words.day_one", locale),
        i18n.get_text("words.day_few", locale),
        i18n.get_text("words.day_many", locale),
    ]
...
def convert_date(
    from_time: Union[str, int], full: bool = True, second: bool = True
) -> Union[str, int]:
    ...
    if str(from_time).isdigit():
        if full:
            to_time = datetime.fromtimestamp(
                from_time, pytz.timezone(BOT_TIMEZONE)
...
def to_number(
    get_number: Union[str, int, float], remains: int = 2
) -> Union[int, float]:
