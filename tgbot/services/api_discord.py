...
class DiscordAPI:
    ...
    async def upload_photo(
        self, photo_data: Union[BytesIO, bytes], photo_name: str = None
    ) -> tuple[bool, str]:
        request_url = f"{self.base_url}/webhooks/{self.webhook_id}/{self.webhook_token}"
        ...
        if "id" in response:
            ...
            photo_url = (
                f"{get_discord_forevercdn}/attachments/{channel_id}/{message_id}"
            )
    ...
    async def _request(
        ...
        try:
            ...
            if response.status == 200:
                ...
                await self.error_account_admin(
...
class DiscordDJ:
    ...
    async def export_webhook(self) -> str:
        ...
        try:
            ...
            await send_errors(
                self.bot, 7729051, f"Error getting Discord Webhook - {ex}"
            )
            return "None"
        else:
    ...
    async def export_forevercdn(self) -> str:
        ...
        try:
            ...
            await send_errors(
                self.bot, 7729051, f"Error getting Discord ForeverCDN - {ex}"
            )
            return "None"
        else:
